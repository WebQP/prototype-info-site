    ...
    <div v-show="showPage" class="container-fluid p-0">
        <div class="row align-items-center mb-2 mb-xl-3">
            ...
            <div class="col-3 text-end">
                <router-link
                    :to="{ name: 'blog-categories-list'}"
                >
                    <i class="fa-duotone fa-arrow-right"></i> рубрики блога
                </router-link>
            </div>
        </div>
        <div class="row">
            ...
            <div class="col-3">
                <div class="card">
                    <div class="card-body">
                        <div v-if="listLanguages.langMain && listLanguages.langMain.prefix === pageLang" class="row align-items-center mb-3">
                            <div class="col-6">
...
export default {
    ...
    methods: {
        ...
        async postPageData(){
            ...
            if(this.id) {
                apiUrl = '/api/blog-category/'+ this.id;
                method = 'PUT'
            }
            ...
            axios.post(apiUrl, {
                ...
                if(response.status === 200){
                    if(this.pageLang === '' || this.pageLangSelect === this.listLanguages.langMain.prefix){
                        window.location.href = "/manager/blog/categories/" + response.data.id;
                    }else{
                        window.location.href = "/manager/blog/categories/" + this.id + '/' + this.pageLangSelect;
                    }
                    this.showPage = false;
                }
            }).catch(error => {
        ...
        getPageData() {
            let apiUrl = '/api/blog-category/'+ this.id +'/edit';
            if(this.pageLang !== ''){
                apiUrl = '/api/blog-category/'+ this.id +'/edit?lang=' + this.pageLang;
            }
            axios
                .get(apiUrl)
                .then(response => {
        ...
        selectLang(){
            if(this.pageLangSelect === this.pageLang){}
                ...
                window.location.href = "/manager/blog/categories/" + this.pageId;
            }else{
                window.location.href = "/manager/blog/categories/" + this.pageId + '/' + this.pageLangSelect;
            }
        },
